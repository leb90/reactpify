    ...
    handleHotUpdate({ file }: { file: string }) {
      ...
      if (fs.existsSync(file)) {
        ...
        console.log(`✅ Auto-generated Liquid template!`);
          ...
          console.log(`\n🔄 COMPONENT UPDATED!`);
          console.log(`📁 ${componentName}.tsx → section.${kebabName}.liquid`);
          console.log(`✅ Liquid template regenerated!`);
        }
      }
    }
  };
...
function generateLiquidTemplate(componentName: string, componentDir: string) {
  ...
  if (!fs.existsSync(reactFilePath)) {
    console.error(`❌ React component not found: ${reactFilePath}`);
    return;
  }

  // Check if the Liquid file was manually edited
  if (isManuallyEditedLiquid(liquidFilePath)) {
    console.log(`\x1b[91m📝 [MANUAL]\x1b[0m Skipping auto-generation for manually edited file: ${liquidFileName}`);
    return;
  }
  ...
  console.log(`✨ Auto-generated/updated Liquid template: ${liquidFileName}`);
}
...
function checkAllComponents() {
  ...
  if (!fs.existsSync(componentsDir)) {
    console.log('📁 Components directory not found, creating...');
  ...
  for (const folderName of componentFolders) {
    ...
    for (const reactFile of reactFiles) {
      ...
      if (!liquidExists) {
        ...
        console.log(`🔄 Regenerating existing template: ${componentName}`);
          generateLiquidTemplate(componentName, componentDir);
      }
    }
  }
