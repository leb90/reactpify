...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    osascript -e 'tell application "Terminal" to do script "echo \"🛍️ SHOPIFY DEV SERVER\"; echo \"Live preview of your theme changes!\"; echo \"\"; npm run dev"'
    ...
    if command -v gnome-terminal &> /dev/null; then
        ...
        sleep 2
        gnome-terminal -- bash -c 'echo "🛍️ SHOPIFY DEV SERVER"; echo "Live preview of your theme changes!"; echo ""; npm run dev; exec bash'
    elif command -v xterm &> /dev/null; then
        xterm -e 'echo "📺 WATCH MODE"; npm run watch' &
        sleep 2
        xterm -e 'echo "🛍️ SHOPIFY DEV SERVER"; npm run dev' &
    else
